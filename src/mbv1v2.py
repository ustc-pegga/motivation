...
def get_model(op,datasets,kernel):
    if op == 'mobilenetv1':
        return MobileNet(n_class=100,kernel_size=kernel)
    elif op == 'mobilenetv2':
        return MobileNetV2(n_class=100,kernel_size=kernel)
    else:
        return None
...
if __name__ == '__main__':
    ...
    data_root = "../paper"
    type = '{}_kernel'.format(op)
    in_c = 3
    input = 224
    kernel_list = [3,5,7,9,11,13,15,17]
    # title
